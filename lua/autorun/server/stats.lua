...
function createTables()
	...
	sql.Query("CREATE TABLE ttt2stats_rounds (id INTEGER PRIMARY KEY AUTOINCREMENT, map TEXT NOT NULL, start_time INTEGER NOT NULL, end_time INTEGER, ended_normally INTEGER (0, 1), winning_team TEXT);")
...
if SERVER then
	...
	hook.Add("PlayerInitialSpawn", "ttt2stats_playerInitialSpawn", function(ply)
		...
		local playerRow = sql.QueryRow("SELECT * FROM ttt2stats_players WHERE steamid = '" .. ply:SteamID64() .. "';")
		if playerRow == nil or playerRow == false then
			if GetConVar("ttt2stats_debug"):GetBool() then
				print("DEBUG-TTT2STATS: Player " .. playerNickname .. " (" .. ply:SteamID64() .. ") not found in ttt2stats_players. Adding them.")
			end
			// Note: playerNickname is already surrounded w/ quotes
			sql.Query("INSERT INTO ttt2stats_players (steamid,friendly_name) VALUES ('" .. ply:SteamID64() .. "'," .. playerNickname .. ");")
			...
			if playerRow["friendly_name"] ~= playerNickname then
				sql.Query("UPDATE ttt2stats_players SET friendly_name = " .. playerNickname .. " WHERE steamid = '" .. ply:SteamID64() .. "';")
			end
		end
	end)
	...
	hook.Add("TTTBeginRound", "ttt2stats_tttbeginround", function()
		...
		sql.Query("INSERT INTO ttt2stats_rounds (map,start_time,ended_normally) VALUES ('" .. mapName .."','" .. startTime .. "', '0');")
		...
		for i = 1, #activePlayers do
			...
			sql.Query("INSERT INTO ttt2stats_player_round_karma (player_steamid, round_id, player_starting_karma) VALUES ('" .. playerSteamID .. "', '" .. roundID .. "', '" .. playerKarma .. "');")
		end
	end)
	
	-- Hook that updates the end_time and ended_normally columns of the round row in ttt2stats_rounds when a round ends.
	hook.Add("TTTEndRound", "ttt2stats_tttendround", function()
		if GetConVar("ttt2stats_debug"):GetBool() then 
			print("DEBUG-TTT2STATS: TTTEndRound hook called.")
		end
		local endTime = os.time()
		sql.Query("UPDATE ttt2stats_rounds SET end_time = '" .. endTime .. "', ended_normally = '1' WHERE id = '" .. roundID .. "';")
		...
		for i = 1, #activePlayers do
			...
			sql.Query("UPDATE ttt2stats_player_round_karma SET player_ending_karma = '" .. playerKarma .. "' WHERE player_steamid = '" .. playerSteamID .. "' AND round_id = '" .. roundID .. "';")
		end
	...
	hook.Add("TTT2UpdateSubrole", "ttt2stats_ttt2updaterole", function(ply, oldSubrole, newSubrole)
		...
		sql.Query("INSERT INTO ttt2stats_player_round_roles (player_steamid,round_id,player_role,role_assign_time) VALUES ('" .. steamID .. "','" .. roundID .. "','" .. newRole .. "','" .. assignTime .. "');")
	end)
	...
	hook.Add("PlayerTakeDamage", "ttt2stats_playerTakeDamage", function(victimEnt, _inflEnt, _attacker, _dmgAmount, dmgInfo)
		if victimEnt:IsPlayer() then
			...
			sql.Query("INSERT INTO ttt2stats_player_damage (round_id,attacker_steamid,victim_steamid,damage_time,damage_dealt,health_remain,weapon) VALUES ('" .. roundID .. "','" .. attackerSteamID .. "','" .. victimSteamID .. "','" .. hurtTime .. "','" .. damageTaken .. "','" .. healthRemaining .. "','" .. weapon .."');")
		end
	end)
end